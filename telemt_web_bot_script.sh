#!/bin/bash
# ============================================
# TELEMT УНИВЕРСАЛЬНЫЙ СКРИПТ
# Версия: 3.4.9+
# Автор: potap1978 & assistant
# ============================================
# Отключаем set -e в начале, обрабатываем ошибки вручную
set +e
...
# ============================================
ensure_swap() {
    local target_mb=2048
    local target_kb=$((target_mb * 1024))

    if [[ -f /swapfile ]]; then
        local file_size_kb=$(du -k /swapfile | cut -f1)
        if [[ "$file_size_kb" -ge "$target_kb" ]] && swapon --show 2>/dev/null | grep -q "/swapfile"; then
            info "Swap ${target_mb}MB уже активен, пропускаю"
            return
        fi
    fi

    warn "Создаю swap ${target_mb}MB..."
    ...
    success "Swap ${target_mb}MB создан и активирован"
}
...
get_primary_tls_domain() {
    get_tls_domains | head -1
}
...
set_tls_domains() {
    ...
    for d in "${domains_array[@]}"; do
        [[ -n "$domains_str" ]] && domains_str="$domains_str, "
    ...
    if grep -q "^\[censorship\]" "$TELEMT_CONFIG"; then
        ...
        echo -e "\n[censorship]\ntls_domains = $domains_str" >> "$TELEMT_CONFIG"
    fi
...
add_tls_domain() {
    ...
    set_tls_domains "${current_domains[@]}"
    success "Домен $new_domain добавлен"
}
...
remove_tls_domain() {
    ...
    local current_domains=()
    while IFS= read -r d; do
        [[ "$d" != "$domain_to_remove" ]] && current_domains+=("$d")
    done < <(get_tls_domains)
    
    if [[ ${#current_domains[@]} -eq 0 ]]; then
        error "Нельзя удалить последний домен"
        return 1
    fi
    
    set_tls_domains "${current_domains[@]}"
    success "Домен $domain_to_remove удалён"
}
...
get_letsencrypt_ip_cert() {
    ...
    step "Получение сертификата на IP $server_ip..."
    if ~/.acme.sh/acme.sh --issue -d "$server_ip" \
        --standalone \
        --server letsencrypt \
        ...
        ~/.acme.sh/acme.sh --install-cert -d "$server_ip" \
            ...
            --fullchain-file /etc/nginx/ssl/telemt-panel.crt
    ...
    if [[ -f "$GEOIP_DB_PATH" ]]; then
        ...
        if [[ -f "$PANEL_CONFIG" ]]; then
            if ! grep -q "^\[geoip\]" "$PANEL_CONFIG"; then
                echo -e "\n[geoip]" >> "$PANEL_CONFIG"
                echo "db_path = \"$GEOIP_DB_PATH\"" >> "$PANEL_CONFIG"
            else
                sed -i "s|db_path = .*|db_path = \"$GEOIP_DB_PATH\"|" "$PANEL_CONFIG"
            fi
            systemctl restart telemt-panel 2>/dev/null
        fi
    else
        warn "Не удалось загрузить GeoIP базу"
    fi
}
...
configure_socks5_upstream() {
    clear
    echo -e "${CYAN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${CYAN}${BOLD}           НАСТРОЙКА SOCKS5 UPSTREAM${NC}"
    echo -e "${CYAN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo -e "${YELLOW}Что такое SOCKS5 Upstream?${NC}"
    echo "  Это позволяет telemt ходить к Telegram API через внешний SOCKS5 прокси"
    echo "  Полезно, если ваш сервер находится в стране с блокировками"
    echo ""
    ...
    case "$socks5_choice" in
        есть|ЕСТЬ)
            ...
            ;;
        установить|install|INSTALL)
            step "Установка Dante SOCKS5 сервера..."
...
EOF
            ...
            socks5_user=""
            socks5_pass=""
            success "Dante SOCKS5 сервер установлен на порту 1080"
            ;;
        *)
            info "Настройка отменена"
            return
            ;;
    esac
...
address = "$socks5_addr"
EOF
        [[ -n "$socks5_user" ]] && echo "username = \"$socks5_user\"" >> "$TELEMT_CONFIG"
        [[ -n "$socks5_pass" ]] && echo "password = \"$socks5_pass\"" >> "$TELEMT_CONFIG"
        echo "enabled = true" >> "$TELEMT_CONFIG"
        echo "weight = 1" >> "$TELEMT_CONFIG"
    fi
    ...
    success "SOCKS5 Upstream настроен!"
    pause
}
...
export_all_links() {
    ...
    echo -e "${GREEN}${BOLD}           ЭКСПОРТ ВСЕХ ССЫЛОК${NC}"
    echo -e "${GREEN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    local current_port=$(grep -oP 'port = \K\d+' $TELEMT_CONFIG 2>/dev/null || echo "7443")
    local server_ip=$(get_server_ip)
    local output_file="/root/telemt_links_$(date +%Y%m%d_%H%M%S).txt"
    
    echo "=== TELEMT ПРОКСИ ССЫЛКИ ===" > "$output_file"
    echo "Сервер: $server_ip" >> "$output_file"
    echo "Порт: $current_port" >> "$output_file"
    ...
    while IFS='=' read -r username secret_part; do
        ...
        if [[ -n "$username" && "$username" != "#"* && "$username" != "temp_user" ]]; then
            echo "Пользователь: $username" >> "$output_file"
            echo "  DD ссылка: tg://proxy?server=$server_ip&port=$current_port&secret=dd${secret}" >> "$output_file"
            echo "  EE ссылка: tg://proxy?server=$server_ip&port=$current_port&secret=ee${secret}" >> "$output_file"
            echo "" >> "$output_file"
        fi
    done <<< "$users"
    
    success "Ссылки экспортированы в файл: $output_file"
    cat "$output_file"
    pause
}
# ============================================
...
[censorship]
tls_domains = ["www.google.com", "cloudflare.com", "microsoft.com"]
...
install_telemt() {
    ...
    if systemctl is-active --quiet telemt; then
        ...
        echo -e "${CYAN}SNI домены по умолчанию:${NC} www.google.com, cloudflare.com, microsoft.com"
...
add_user() {
    ...
    echo -e "${CYAN}Текущие SNI домены сервера:${NC}"
    get_tls_domains | while read d; do echo -e "  ${YELLOW}$d${NC}"; done
    ...
    if [[ $users_count -gt 1 ]] && grep -q "^temp_user = " $TELEMT_CONFIG; then
        ...
        info "Тестовый пользователь temp_user автоматически удалён"
    fi
...
list_users() {
    ...
    echo -e "${CYAN}Глобальные SNI домены сервера:${NC}"
    get_tls_domains | while read d; do echo -e "  ${YELLOW}$d${NC}"; done
...
manage_sni_domains() {
    ...
    echo -e "${MAGENTA}${BOLD}           УПРАВЛЕНИЕ SNI ДОМЕНАМИ${NC}"
    echo -e "${MAGENTA}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    echo -e "${CYAN}Текущие SNI домены:${NC}"
    local domains=()
    while IFS= read -r d; do
        domains+=("$d")
        echo -e "  ${YELLOW}$d${NC}"
    ...
    echo "  3) Установить список по умолчанию (google, cloudflare, microsoft)"
    echo "  0) Назад"
    echo ""
    read -p "Выбор: " sni_choice
    
    case $sni_choice in
        1)
            read -p "Введите домен для добавления: " new_domain
            if [[ -n "$new_domain" ]]; then
                add_tls_domain "$new_domain"
            fi
            ;;
        2)
            echo -e "${CYAN}Выберите домен для удаления:${NC}"
            for i in "${!domains[@]}"; do
                echo "  $((i+1))) ${domains[$i]}"
            done
            read -p "Номер: " del_num
            if [[ "$del_num" =~ ^[0-9]+$ ]] && [[ $del_num -ge 1 ]] && [[ $del_num -le ${#domains[@]} ]]; then
                remove_tls_domain "${domains[$((del_num-1))]}"
            else
                error "Неверный номер"
            fi
            ;;
        3)
            set_tls_domains "www.google.com" "cloudflare.com" "microsoft.com"
            success "Установлены домены по умолчанию"
            ;;
        *) return ;;
    esac
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "status":
        ...
        await query.edit_message_text("🤖 *Telemt Bot v3.4.9*\n\n*Передай привеД ПОТАПу !!!*\n\nВыберите действие:", reply_markup=get_main_keyboard(), parse_mode='Markdown')

    elif data == "change_sni":
        user_inputs[user_id] = {'action': 'change_sni'}
        await query.edit_message_text("Введите новый SNI домен (например: cloudflare.com):", reply_markup=get_cancel_keyboard())
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'add_user':
        if not re.match(r'^[a-zA-Z0-9_-]+$', text):
            ...
            await update.message.reply_text("🤖 *Telemt Bot v3.4.9*\n\n*Передай привеД ПОТАПу !!!*\n\nВыберите действие:", reply_markup=get_main_keyboard(), parse_mode='Markdown')
            del user_inputs[user_id]
            return
        existing_users = get_users()
        for u in existing_users:
...
install_telemt_panel() {
    ...
    echo "  1) Самоподписной (простой, браузер ругается)"
    echo "  2) Let's Encrypt на IP (валидный, срок 6 дней) — НОВИНКА!"
    echo "  3) Использовать мои сертификаты"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    ...
    read -sp "Введите пароль администратора панели: " panel_password
    echo ""
...
EOF
    ...
    echo "  • GeoIP база загружена (видна страна клиентов)"
    echo ""
...
update_telemt_panel() {
    ...
    if systemctl is-active --quiet telemt-panel; then
        ...
        echo -e "${GREEN}✅ GeoIP база обновлена${NC}"
...
show_status() {
    ...
    if systemctl is-active --quiet telemt-panel 2>/dev/null; then
        ...
        if [[ -f "$GEOIP_DB_PATH" ]]; then
            echo -e "${CYAN}   GeoIP: ${GREEN}активна${NC}"
        else
            echo -e "${CYAN}   GeoIP: ${RED}не установлена${NC}"
        fi
    else
    ...
    if grep -q "^\[\[upstreams\]\]" "$TELEMT_CONFIG" 2>/dev/null; then
        ...
        if [[ -n "$socks5_addr" ]]; then
            echo -e "${CYAN}● SOCKS5 Upstream:${NC} ${GREEN}$socks5_addr${NC}"
        fi
    fi
...
show_menu() {
    ...
    echo -e "  ${BLUE}7)${NC} Управление SNI доменами (несколько доменов)"
    echo -e "  ${BLUE}8)${NC} Сменить порт"
    echo -e "  ${BLUE}9)${NC} Изменить лимит IP для пользователя"
    echo ""
    echo -e "${MAGENTA}  ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ${NC}"
    echo -e "  ${MAGENTA}10)${NC} Настройка SOCKS5 Upstream"
    echo -e "  ${MAGENTA}11)${NC} Экспорт всех ссылок пользователей"
    echo ""
    echo -e "${CYAN}  ИНФОРМАЦИЯ${NC}"
    echo -e "  ${CYAN}12)${NC} Статус и информация"
    echo ""
    echo -e "${MAGENTA}  TELEGRAM БОТ${NC}"
    echo -e "  ${MAGENTA}13)${NC} Установить Telegram бота"
    echo -e "  ${RED}14)${NC} Удалить Telegram бота"
    echo ""
    echo -e "${MAGENTA}  WEB ПАНЕЛЬ${NC}"
    echo -e "  ${MAGENTA}15)${NC} Установить Web панель (с GeoIP и LE на IP)"
    echo -e "  ${RED}16)${NC} Удалить Web панель"
...
main() {
    ...
    while true; do
        ...
        case $choice in
            ...
            5) show_qr_menu ;;
            6) remove_user ;;
            7) manage_sni_domains ;;
            8) change_port ;;
            9) change_user_limit ;;
            10) configure_socks5_upstream ;;
            11) export_all_links ;;
            12) show_status ;;
            13) install_bot ;;
            14) uninstall_bot ;;
            15) install_telemt_panel ;;
            16) uninstall_telemt_panel ;;
            17) change_panel_credentials ;;
            18) update_telemt_panel ;;
            19) update_geoip_db ;;
            20) get_letsencrypt_ip_cert "$(get_server_ip)" ;;
            21) update_telemt ;;
            0) 
                clear
                info "До свидания!"
                ...
                ;;
            *) 
                error "Неверный выбор. Пожалуйста, выберите от 0 до 21"
                sleep 2
                ;;
        esac
    done
