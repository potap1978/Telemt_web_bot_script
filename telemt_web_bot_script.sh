...
ensure_swap() {
    ...
    if [[ -z "$swap_total" || "$swap_total" == "0" || "$swap_total" -lt "$target_mb" ]]; then
        warn "Swap ${swap_total:-0}MB < ${target_mb}MB. Пересоздаю..."
        if [[ -f /swapfile ]]; then
        ...
        info "Swap ${swap_total}MB — достаточно (>= ${target_mb}MB)"
    fi
}
