...
ensure_swap() {
    ...
    if [[ -z "$swap_total" || "$swap_total" == "0" ]]; then
        warn "Swap не обнаружен. Создаю 2GB swap..."
        fallocate -l 2G /swapfile 2>/dev/null || dd if=/dev/zero of=/swapfile bs=1M count=2048
        chmod 600 /swapfile
        mkswap /swapfile
        swapon /swapfile
        grep -q "/swapfile" /etc/fstab || echo "/swapfile none swap sw 0 0" >> /etc/fstab
        success "Swap 2GB создан и активирован"
    elif [[ "$swap_total" -lt 2048 ]]; then
        warn "Swap ${swap_total}MB < 2048MB. Увеличиваю до 2GB..."
        ...
        swapon /swapfile
        grep -q "/swapfile" /etc/fstab || echo "/swapfile none swap sw 0 0" >> /etc/fstab
        success "Swap увеличен до 2GB"
    else
        info "Swap ${swap_total}MB — достаточно (>= 2GB)"
    fi
}
